...
-(void) addBtnClick
{
    ...
    self.option.name = self.nameField.text;
    self.option.phoneNumber = self.phoneField.text;
    if ([self.delegate respondsToSelector:@selector(editViewController:optionData:)]) {
        [self.delegate editViewController:self optionData:self.option];
    }
}
-(void)textChange
{
